...
struct DrugAndDropCheck: View {
    ...
    var body: some View {
            ...
            Button(
                action: {
                    if let pdfURL = createPDF(from: capturedMediaDoc) {
                        ...
                        navigationPath.append(Destination.createcheck) // Переход к CreateDocumentView
                    }
                },
                label: {
                    Text("Продолжить")
        ...
        .navigationBarTitle("Формирование чека")
    }
